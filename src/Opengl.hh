...
EXPORT_EXTENSION(glDeleteBuffers, GLDELETEBUFFERS);
EXPORT_EXTENSION(glBindBuffer, GLBINDBUFFER);
EXPORT_EXTENSION(glBufferData, GLBUFFERDATA);
EXPORT_EXTENSION(glBufferSubData, GLBUFFERSUBDATA);
EXPORT_EXTENSION(glVertexAttribPointer, GLVERTEXATTRIBPOINTER);
EXPORT_EXTENSION(glGetBufferParameteriv, GLGETBUFFERPARAMETERIV);
...
EXPORT_EXTENSION(glDrawRangeElements, GLDRAWRANGEELEMENTS);
EXPORT_EXTENSION(glMultiDrawElements, GLMULTIDRAWELEMENTS);
EXPORT_EXTENSION(glPrimitiveRestartIndex, GLPRIMITIVERESTARTINDEX);
